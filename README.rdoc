...
To develop Paperclip, download it, make sure the tests pass, and local install it:

  git clone <paperclip>
  sudo gem install jferris-mocha # test dependency
  sudo gem install ruby-debug # test dependency
  sudo gem install aws-s3 # test dependency for amazon-web-services
  cd paperclip
  rake install

==CouchRest Integration

CouchDB is a fantastic Database system.  Paperclip will soon support not only ActiveRecord, but the great CouchRest ORM for CouchDB.

Currently, it will just work without any validations.  Next is to make sure the validations work and to write some tests.